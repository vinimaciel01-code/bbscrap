...
def altera_variaveis(dados):
    ...
    # MES_REF
    # troca "próxima fatura" pelo mes da fatura
    ...
    temp['mes_ref'] = temp['mes_ref'].apply(lambda x: dt.datetime.strptime(x, '%b/%y').date())
    temp['mes_ref'] = temp['mes_ref'] + relativedelta(months=1)
    temp['mes_ref'] = [dt.datetime.strftime(x, '%b/%y') for x in temp['mes_ref']]
    
    # substitui
    for index, row in temp.iterrows():
        data_base.loc[(data_base.conta == index), 'mes_ref'] = row['mes_ref']
